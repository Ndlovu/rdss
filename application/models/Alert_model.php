...
/*    public function show_alerts(){
            $sql ="SELECT alert_id, age, sex, status, user_id as uid, report_date, disease_id as did,facility_id as fid, (SELECT parent_id FROM facility_table WHERE facility_id = fid) as sub_id, (SELECT disease_name FROM disease_table WHERE disease_id = did) as disease_name, (SELECT parent_id FROM sub_county_table WHERE sub_county_id = sub_id) as cid,(SELECT county_name FROM county_table WHERE county_id = cid) as c_name, (SELECT names FROM user_table WHERE user_id = uid ) AS user_name FROM alerts_table";
